...
This project repackages Anthropic's **official Linux `.deb`** (apt repo `https://downloads.claude.ai/claude-desktop/apt`); it bundles its own Electron (44.4.3 as of v2.7032.0) and a native Cowork VM backend. We download it, verify it, extract its `app.asar`, apply our patches, and repackage for Arch/Fedora/RHEL/Nix/AppImage + our own Debian/Ubuntu `.deb`.
