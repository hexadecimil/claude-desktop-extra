...
**claude-desktop-extra** (`/home/patrickjaja/development/claude-desktop-extra/`)
   Repackages Anthropic's **official Claude Desktop Linux `.deb`** (apt repo `https://downloads.claude.ai/claude-desktop/apt`; bundles its own Electron (44.4.3 as of v2.7032.0) and a native Cowork VM backend) as native packages for the distros Anthropic does not ship (Arch via our own pacman repo, Fedora/RHEL, NixOS, AppImage) plus our own Debian/Ubuntu `.deb`.
