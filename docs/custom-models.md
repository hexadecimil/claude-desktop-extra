...
Community feature, **off by default**: **Settings → Extra → Community Features → Custom models** (or **Turn on** in the Models panel) is the switch, **Settings → Extra → Models** the editor. Off leaves the picker and the sessions exactly as Anthropic ships them - no debugger on the page, nothing added to a session. [What it changes in the app's traffic](#what-it-changes-in-the-apps-traffic) lists the local edits it makes.

## Setup from the app

**Settings → Extra → Models → Add a provider.** Pick a preset (DeepSeek, Kimi, GLM, MiniMax, Qwen, OpenRouter, or a gateway) or fill in a provider id and the endpoint's base URL by hand, paste the API key, **Add provider**. A preset knows the endpoint, where the provider lists its models, which effort levels its API accepts and, when every model it serves has it, the context window - not the models themselves, which age too fast: **Fetch models** on the card asks the provider for its list and you tick the ones to add - a filter box narrows a long list, and a listing that states each model's context length (OpenRouter's) sets the added model's [context window](#context-window) by itself (it also works for an unknown provider whose `/v1/models` follows the OpenAI shape). **test** sends one token with the stored key, so a wrong URL or key shows there rather than as a failed session; **detect** (in the provider form) asks which effort levels it accepts. A models list URL must be on the base URL's host, and a base URL moved to another host drops the stored key and custom headers - see [Where the key goes](#where-the-key-goes). **Add a model** / **edit** set the model id, the display name, images, thinking, the web search tool, the default effort, the [context window](#context-window), an optional badge, and the model's [sub-agent type](#sub-agents) - the name Claude launches it under, on by default. **Turn on** the feature, then **Reload the Code tab** (the button next to the switch, or a restart): the picker lists the new entries after the Claude models. The routing itself is live - see [What reaches open sessions](#what-reaches-open-sessions).
...
2. Add a `customModels` block (`"enabled": true` turns the feature on and locks the Settings switch; leave it out to use the switch):
...
## What it changes in the app's traffic

Besides sending the sessions that pick a custom model to its provider, the feature edits some traffic
locally:

- the picker's `/api/bootstrap` response gets the custom entries;
- the page's model-selection write for a custom id is answered locally, since claude.ai refuses an id it
  does not know, so the choice is remembered;
- requests for Claude models go to Anthropic unchanged, with two exceptions. After a turn answered by a
  custom model, `diagnostics.previous_message_id` is sent as null when it holds the other provider's
  message id, which Anthropic would refuse. When the web-search model is an Anthropic one, the web-search
  sub-request's model name is replaced by it.

## Reference

```jsonc
"customModels": {
  "enabled": true,               // the switch. Absent = off, like every community feature.
  ...
  "smallFastModel": "deepseek-flash", // the CLI's "small/fast model" slot (ANTHROPIC_SMALL_FAST_MODEL):
                                 ...
                                 // this model. A custom model (id or claude-<id>; one without the web
                                 // search tool needs `webSearch` above set), or any Anthropic id, or
                                 // absent for the CLI's own small default. See Small / fast model
  "subagentModel": "deepseek-flash", // optional: the model of every sub-agent that names none - the CLI's
                                 // own general-purpose and Plan included, not Explore, which the CLI pins
                                 // to the session's model (CLAUDE_CODE_SUBAGENT_MODEL).
  ...
  "providers": [
    {
      ...
      "modelsUrl": "https://api.deepseek.com/v1/models", // where "Fetch models" asks (optional, found automatically;
                                                         // the panel requires the base URL's host)
      ...
      "effortMap": { "xhigh": "high" },                  // optional, overlays the level table, see Effort below
      "models": [
        {
...
The target is Anthropic-**compatible**, not Anthropic, so the CLI's request is reduced to the documented subset of the Messages API: `model`, `max_tokens`, `stream`, `temperature`, `top_p`, `stop_sequences`, `system` (text blocks), `messages` (text, image, tool_use, tool_result, thinking, native web search), `tools` (name/description/input_schema, plus the server-side web search tool with its `max_uses`, domain filters and `user_location`), `tool_choice`, `thinking`, `output_config.effort`. Dropped: `metadata`, `context_management`, `top_k`, every `cache_control`, MCP toolsets, `document` blocks other than plain text (replaced by a one-line placeholder), the `anthropic-beta` headers and the Anthropic OAuth `Authorization` (the provider gets `x-api-key` only). A redirect is followed only while it stays on the provider's host. Mid-conversation `system` messages (the CLI's `/effort` and friends) are folded into the next user turn as `[system] …`, and turns are merged to keep the strict user/assistant alternation. `count_tokens` is estimated locally (chars / 4) - the compatible endpoints do not serve it.

**Effort.** The picker's effort menu lists the levels the provider accepts (`providers[].effort`, default all five: low, medium, high, xhigh, max - an API convention of the provider, inherited by its models). A provider given an explicit list - by the panel, the file or its preset - gets the chosen level unchanged as `output_config.effort`; a level the list lacks (the CLI's `/effort` can name any) goes to the nearest listed one above, else the highest. DeepSeek knows `low`, `high`, `max`, so its preset sets exactly those and the default lands on `max` (the setting its benchmarks ran at). A provider left on the default five gets DeepSeek's convention instead: `medium` and `xhigh` are sent as `high` and `max`. `effortMap` (per provider) overlays either table for the rare API that spells a level differently. **detect** in the provider form asks the provider itself - one minimal request with each level to its first model (thinking on, the 1024-token minimum budget, so at most about a thousand output tokens each) - and ticks what it accepted; that settles strict APIs (a refused name is a 400) but not permissive ones: DeepSeek answers 200 to all five names and folds the extra ones (`xhigh` → `high` per its docs), which is exactly why its preset lists three. No provider exposes its list of levels - the preset is the documentation, detect the experiment, and the run-time fallback the safety net. Should a provider still refuse a level, the request is retried without effort and the refusal is logged. Effort never travels with thinking off (DeepSeek answers 400 to that).

**Thinking** follows the request. The CLI asks for it on a session's turns (`enabled` with a budget, or `adaptive`) and leaves it out of its light work (WebFetch's synthesis, the classifier), which stays without thinking and without effort. A model marked `thinking: false` never gets it. The budget is clamped to 16 000 tokens and kept below `max_tokens`; a `max_tokens` too small for the 1024-token minimum sends no thinking rather than an invalid request.

**Switching models mid-session.** A history that carries thinking blocks signed by Claude may be refused by the provider with a 400 mentioning the signature; the request is retried without those blocks. The two retries add up (at most two): a turn whose history is signed by Claude *and* whose effort level the provider lacks goes through on the second one.

**Threads.** When the CLI's server-side threads are on for your account, it sends a turn as the continuation of an earlier one - only the messages after an anchor, sometimes without the tools, the rest being kept by Anthropic. A provider never saw that anchor, so such a request is refused with the error code the CLI answers by resending the turn whole and keeping threads off for that model for the rest of the session. It costs nothing but a local round trip. A provider 401/403 (wrong or revoked key) is turned into a plain 400 (`invalid_request_error`) carrying the provider's message, because a 401 would make the CLI refresh its own OAuth token and retry without end, and an `authentication_error` type would make the app re-authorise its own login and restart the session's CLI; other errors are returned to the CLI as they are.

### What reaches open sessions

Two different things change at two different times:

- **Routing** (a key pasted or fixed, a model added or removed, a provider removed, the web-search target, the switch) is live: the app rewrites `custom-models/routes.json` at every change - the panel's writes, and hand edits of the `.jsonc`/`.json` or of `secrets.json`, which are watched - and every open session re-reads it on its next request. A wrong key corrected in the panel works on the very next message; `/model claude-<new id>` and a workflow naming a new model work without a restart. A key read from an `apiKeyFile` or `apiKeyEnv` is re-read when a session opens or the config changes, not when that file changes.
...
- The [small/fast model](#small--fast-model) and the [default sub-agent model](#sub-agents) are set in a session's environment when it opens, and the CLI keeps them for its whole life: those two keep their route in that session even after the feature is turned off or the model removed, until the session ends.
- A session opened while the feature was off (or before your first provider existed) was spawned without the preload - the feature contributes nothing to such a session - so it needs reopening once the feature is on.

A request for a custom model whose route is gone (removed, or the feature turned off) is answered with a clear error naming the model, instead of reaching Anthropic, which does not know it. Deleting a model also clears the web-search, small/fast and default sub-agent choices that named it.
...
- the **web-search sub-request** *when `webSearch` is unset*, on accounts where Claude Code sends it on
  this slot (a server-side flag; otherwise it goes on the session's model) - once `webSearch` names a
  target, the search runs there whatever model the sub-request names.

Point the slot at a custom model (`smallFastModel`, the id or `claude-<id>`) or any Anthropic id, or
leave it on the CLI's own small default. Any configured model qualifies, with one catch: while no
web-search model is set, a web search goes to this slot or to the session's model, so a model marked
without the web search tool would have to answer a search it cannot run. Set a web-search model then;
the panel warns in the web-search row, and such a search is refused with that reason rather than
answered without searching. Because the slot is a CLI env var, the setting takes effect
the next time a session opens - already-open sessions keep what they got, route included (the same as
`subagentModel`).
...
Because the types and the line travel in the session's `initialize` request and the default sub-agent model in its environment at spawn, a session already open keeps what it got
(see [What reaches open sessions](#what-reaches-open-sessions)). Only local Code sessions get them - the kind
that has the preload; Cowork, Dispatch and SSH sessions never see a type they could not run.

### Where the key goes

To the provider, from two places only: the Code session's CLI, which reads it from `custom-models/routes.json` (0600 in a 0700 directory, the same exposure as `secrets.json` next to it, re-read for live changes) - never from its environment, which carries the routes without their keys - and the app itself, for the panel's **test**, **Fetch models** and **detect** buttons. It never reaches the page: the picker entries carry names and ids only. The log never prints it.

It only ever goes to the host of the base URL it was stored for. The Models panel is driven by the page (remote claude.ai code), so the page must not be able to steer a key elsewhere: a base URL moved to another host drops the stored key (and any key source a hand edit left in the `.json`) unless a new key is typed in the same save - the panel then asks for it again - as well as the provider's custom `headers`, which can hold a gateway's credential; a models list URL must be on the base URL's host; and a redirect to another host is not followed. A provider of the hand-owned `.jsonc` is exempt from the second rule: the page cannot write that file.

## How it works

Nothing local feeds the Code tab's model menu: the page (remote claude.ai code) builds it from `model_selector_config` in the `/api/bootstrap` response and then tells the app which ids exist. `modelPicker` in `~/.claude/settings.json` only reaches the CLI's own `/model`. So the patch (`patches/community/add_feature_custom_models.nim`) does four things:
...
- **Routing.** The Claude Code binary is a compiled Bun program and honours `BUN_OPTIONS`. `cliEnv()` is spliced into `buildSessionEnv`, where the Code session manager copies the session environment at every spawn (sub-patch B; not the base env builder, which upstream caches for 30 seconds), builds on the env upstream is about to send - your Code-tab environment variables included - and adds `--preload=<userData>/custom-models/preload.js` - `js/custom_models_preload.js`, written there by the app - plus the providers without their keys in `CDB_CUSTOM_MODELS_JSON` and the path of `routes.json` in `CDB_CUSTOM_MODELS_ROUTES`. A profile path with a space in it cannot ride in `BUN_OPTIONS`; the session is then left alone and the log says why. `CLAUDE_CODE_SUBAGENT_MODEL` (the default sub-agent model) and `ANTHROPIC_SMALL_FAST_MODEL` (the [small/fast slot](#small--fast-model) - WebFetch and the mid-turn classifier) ride along when set. The preload replaces `fetch` inside the CLI, forwards only requests whose model is one of ours, and re-reads `routes.json` (one `stat` per API request) when the app rewrites it; when the feature is off it contributes nothing and the session is exactly upstream's. A `BUN_OPTIONS` the app inherited from its environment is kept, our `--preload` appended once: any other preload named there runs in the CLI too, and a leftover `fetch` hook from an earlier setup can quietly serve what this one leaves alone - `CDB_CUSTOM_MODELS_DEBUG=1` shows the `passthrough` and a fresh `-p` run of the CLI with only our preload shows the error the CLI gives on its own.

- **Sub-agents.** The app opens every Code session with an `initialize` request to the CLI that carries, among other fields, `appendSystemPrompt` and `agents` - the Agent SDK's system-prompt suffix and programmatic sub-agent definitions. Sub-patch C routes both through `agents()` and `appendSystemPrompt()` of the main module (two literal anchors in the code-split chunk, each expected exactly once): one definition per model with a type, `{description, prompt, model: <listed id>}`, and the announce line. The CLI accepts any `--model`-valid id in a definition's `model` (checked 2026-09-14, CLI 2.1.266, with `--agents`), and honours `CLAUDE_CODE_SUBAGENT_MODEL` with such an id too. That request belongs to the Agent SDK's generic query, which Cowork, Dispatch and the app's helper queries go through too, so the query's transport is passed along: only a session whose spawn environment carries `CDB_CUSTOM_MODELS_JSON` - one `cliEnv()` fed, which has the preload - gets anything. Off, or any other session: both functions return the app's own values untouched.
- **Remote sessions.** An SSH (or WSL) Code session starts from the same session environment, but its CLI runs on another host, without the preload, and the remote spawner forwards every `CLAUDE_*`/`ANTHROPIC_*` variable. Sub-patch D calls `remoteSpawn()` at the start of the app's `configureSSHSpawn`, which takes everything `cliEnv()` added back out, restoring what those variables held before: the remote session gets exactly the environment upstream would send. (The keys never reached a remote host - the forwarder drops `CDB_*` variables - but the two model slots would have named ids nothing routes there.)

Limits: local Code sessions only; the environment and session setup of Cowork, Dispatch and SSH sessions stay as Anthropic ships them. The picker itself is shared, so a custom model picked in such a session is refused by it. Whether a given provider's `/anthropic` endpoint supports a request shape (images, streaming, tools) is the provider's business; the log shows its answer. Anthropic's server-side `web_search` tool is served by DeepSeek's and OpenRouter's endpoints (verified 2026-09); a provider that refuses it answers 400 and the log shows it.

Debugging: `CDB_CUSTOM_MODELS_DEBUG=1` in the app's environment logs every API request the page makes (path only), the surfaces of each bootstrap and the `cliEnv()` calls, all under `[custom-models] debug:` in `logs/claude-patches.log` (the `selection: … kept locally` lines are always there). The switch is passed to the sessions' CLI too: `logs/custom-models.log` then also gets one `passthrough <url> model=<id> (<reason>)` line per Messages request that was left alone (a Claude model, or a gateway's own `claude-*` id whose route is gone), next to the `->` lines of the routed ones.

What the feature relies on and no build can check - the claude.ai APIs, the CLI's internals - is inventoried, with the log line that says which one moved, in [`baseline/CUSTOM_MODELS_ANCHORS.md`](../baseline/CUSTOM_MODELS_ANCHORS.md).
